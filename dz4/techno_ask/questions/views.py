...
def questions_by_tag(request, tag, page='1'):
    ...
    return render_to_response('question_by_tag.html',
                              {'tag': tag, 'questions': questions, 'tags': randomTags(Tag), 'users': randomUsers(User),
                               'isAuthorized': True})
...
def questions_by_user(request, user, page='1'):
    ...
    return render_to_response('question_by_user.html', {'user': user, 'questions': questions, 'tags': randomTags(Tag),
                                                        'users': randomUsers(User), 'isAuthorized': True})


def question(request, id):
    question_ = get_object_or_404(Question, pk=id)
    answers = question_.answer_set.all()
    return render_to_response('question.html', {'question': question_, 'answers': answers, 'tags': randomTags(Tag),
                                                'users': randomUsers(User), 'isAuthorized': True})
...
def settings(request):
    return render_to_response('settings.html',
                              {'tags': randomTags(Tag), 'users': randomUsers(User), 'isAuthorized': True})
